...
data_tbl <- data_tbl %>% 
  ...
  mutate(Alarm = case_when(
    ...
    Alarm == "SpO2" & Richtung == "überschritten" ~ "SpO2.high",
    Alarm == "SpO2" & Richtung == "unterschritten" ~ "SpO2.low",
...
# Some parameters do not have red AND yellow thresholds, but store them as two distinct parameters. This is the case for HF.high and xTachy (xTachy = red alarms of HF.high), SpO2.low and Desat (Desat = red alarms of SpO2.low), HF.low and xBrady (xBrady = red alarms of HF.low) and the generic alarms of the ventilator (OPERATOR = yellow and GENERIC ALARM = red). To make the following figure clinically more meaningful, these parameters are subsummized into one. Additionally, all parameters from the ventilator are labeled with an asterisk.

# store the changes in a temporary object that can be deleted after plotting
data_tbl_temp <- data_tbl

# convert the Alarm column to character for mutating
data_tbl_temp$Alarm <- as.character(data_tbl_temp$Alarm)

data_tbl_temp <- data_tbl_temp %>%
  mutate(Alarm = ifelse( # add asterisk to alarm names of the ventilator
    Alarmgruppe == "Ventilator",
    sprintf("*%s", Alarm),
    Alarm
  )) %>% 
  mutate(Alarm = ifelse(
    Alarm == "xTachy", "HF.high", # subsummize these parameters
    ifelse(
      Alarm == "xBrady", "HF.low",
      ifelse(
        Alarm == "Desat", "SpO2.low",
        ifelse(
          Alarm == "*OPERATOR", "*ALLG. ALARM",
          Alarm # leave all other alarms as they are
        )
      )
    )
  ))
...
data_tbl_temp$Alarm <- as.factor(data_tbl_temp$Alarm)
...
# Translate German alarm names to English
source("TranslateAlarms.R")
...
*Figure 1*. Frequency of individual alarm parameters with more than 500 occurences in the data. The colors correspond to the alarm criticalities (red, yellow, and blue).
...
**Table 6: Distribution of the Alarm Durations**
```{r inspecting duration distribution, message=FALSE, warning=FALSE}
# Inspecting the distribution of the durations
# pulling the duration time values from the data
Rts.clinical <- data_tbl %>%
  filter(Situation == "beendet") %>%
  filter(!(Alarm %in% tech.Alarms.color$Alarm)) %>% 
  select(Zeitdifferenz) %>%
  filter(!is.na(Zeitdifferenz)) %>% 
  pull()

Rts.technical <- data_tbl %>%
  filter(Situation == "beendet") %>%
  filter(Alarm %in% tech.Alarms.color$Alarm) %>% 
  select(Zeitdifferenz) %>%
  filter(!is.na(Zeitdifferenz)) %>% 
...
summary(Rts.clinical)
summary(Rts.technical)
```
*Table 6*: The average duration of clinical alarms is `r round(summary(Rts.clinical)[4], 2)`seconds (SD `r round(sd(Rts.clinical), 2)`s, Median `r round(median(Rts.clinical), 2)`s). The average duration of technical alarms is `r round(summary(Rts.technical)[4], 2)`seconds (SD `r round(sd(Rts.technical), 2)`s, Median `r round(median(Rts.technical), 2)`s). The distribution of the alarm durations is heavily skewed, which is why the median will be used for plotting instead of the mean. Additionally, because some alarm durations were calculated to be longer than 4 days, all alarm durations longer than 8 hours (approx. the length of one shift) are considered as outliers.


```{r plotting the median duration per device, message=FALSE, warning=FALSE}
# general overview of RT center and spread per device and alarm color of RT < 8 hours (28800 seconds)
RT.device <- aggregate(Zeitdifferenz ~ Alarmgruppe + Alarmfarbe,
                       data = data_tbl[which( data_tbl$Situation == "beendet" & data_tbl$Zeitdifferenz <= 28800 ),], FUN = function(x) 
                         c(mean = mean(x), SD = sd(x), min = min(x), max = max(x), median = median(x) ) ) 
RT.device[,3] <- round(RT.device[,3], 2)

# overview of RT center and spread for the alarm colors of RT < 8 hours (28800 seconds)
RT.color <- aggregate(Zeitdifferenz ~ Alarmfarbe,
                       data = data_tbl[which( data_tbl$Situation == "beendet" & data_tbl$Zeitdifferenz <= 28800 ),], FUN = function(x) 
                         c(mean = mean(x), SD = sd(x), min = min(x), max = max(x), median = median(x) ) ) 

# overview of RT center and spread for the medical devices irregardless of color (of RT < 8 hours (28800 seconds))
...
medianRT.device %>%
  ...
  annotate("text", x = 2.2 , y = 2, label = "n/a", size = 3, family = "Times New Roman") +
  annotate("text", x = 5.2 , y = 2, label = "n/a", size = 3, family = "Times New Roman") +
...
*Figure 5*. The median duration of alarms from eight medical devices. The colors correspond to the alarm criticalities (red, yellow and blue). NIBP=noninvasive blood pressure; SpO2=Oxygen saturation; ECG=electrocardiogram; ICP=intracranial pressure; IBP=invasive blood pressure.


```{r plotting median duration against frequency of alarms by device, message=FALSE, warning=FALSE}
...
RTxFreq %>%
  ...
  scale_x_continuous(expand = expand_scale(mult = c(0, .015)), breaks = seq(0, 130, by = 5)) +
...
*Figure 6*. The median alarm duration from eight medical devices plotted against the total number of alarms issued by the respective device. The colors correspond to the alarm criticalities (red, yellow and blue). NIBP=noninvasive blood pressure; SpO2=Oxygen saturation; ECG=electrocardiogram; ICP=intracranial pressure; IBP=invasive blood pressure. 
...
RT.device.time %>%
  ...
  labs(y = "Median Alarm Duration in Seconds",
       x = "Time of Day",
       color = "",
       title = "",
       caption = "") +
...
```{r how many pauses were "proper pauses"?, message=FALSE, warning=FALSE,results="hide"}

ProperPause_tbl <- data_tbl %>% 
  ...
  arrange(Zeit) %>% 
  # deleting time differences not related to pauses and pauses terminated after 190s as the longest possible pause length is 180s.
  # The cut-off is 190s instead of 180s sharp, because of the lag time of the time entry.
  mutate(PausenDauer = ifelse(DummyPause != "Pause", NA, PausenDauer)) %>%
  mutate(PausenDauer = ifelse(Alarmpause == "Pause Aus" & (PausenDauer > 190 | PausenDauer < 0), NA, PausenDauer)) %>% 
  select(-DummyPause) %>% # removing the DummyPause column

  # Marking "proper" and "improper" pauses, as well as pauses that are part of a "continuous pause" (pauses re-enabled within 180 seconds after the last pause terminated).
 
  filter(!is.na(Alarmpause)) %>% # first: filtering all entries unrelated to Pauses, to allow the lead() function below
  
  # Creating a variable that indicates whether a pause is proper or not
  mutate(ProperPause = case_when(
    Alarmpause == "Pause Aus" & PausenDauer >= 180 ~ FALSE, # Pause terminated after 180s, therefore improper
    Alarmpause == "Pause Aus" & PausenDauer < 180 ~ TRUE # # Pause terminated before 180s, therefore improper
  )) %>% 
  
  # creating a variable that indicates whether a pause is part of a continous pause
  mutate(ContPause = case_when(
    Alarmpause == "Pause Ein" & PausenDauer <= 180 ~ TRUE, # Pause re-ebnabled within 180s, therefore part of a continuous pause
    Alarmpause == "Pause Ein" & PausenDauer > 180 ~ FALSE # # Pause re-ebnabled after 180s, therefore not part of a continuous pause
  )) %>% 
  
  # If a pause is part of a continuous pause, it can be neither proper not improper, because it is "one long pause".
  # Here, entries of the ProperPause variables are changed to NA accordingly
  mutate(ProperPause = case_when(
    ProperPause == FALSE & lead(ContPause) == FALSE ~ FALSE, # "The pause is not proper and it's not part of a continuous pause (CP), therefore it is stored as not proper"
    ProperPause == FALSE & lead(ContPause) == TRUE ~ NA, # "The pause is not proper but it's part of a CP, therefore it is erased"
    ProperPause == TRUE & lead(ContPause) == FALSE ~ TRUE, # "The pause is proper and it's not part of a CP, therefore it remains a proper pause"
    ProperPause == TRUE & lead(ContPause) == TRUE ~ NA, # "The pause is proper but it's part of a CP, therefore it is erased"
    ProperPause == FALSE & is.na(lead(ContPause)) ~ FALSE, # "The pause is not proper and there is no info whether it is part of a CP, therefore it remains not proper"
    ProperPause == TRUE & is.na(lead(ContPause)) ~ TRUE # "The pause is proper and there is no info whether it is part of a CP, therefore it remains proper"
  )) %>% 
  
  # selecting only those variables that are relevant for the subsequent analysis
  select(Bettname, Zeit, Alarmpause, PausenDauer, ProperPause, ContPause)


# proper pause to pause ratio
PP.summary <- ProperPause_tbl %>%
  filter(!is.na(ProperPause)) %>% 
  group_by(ProperPause) %>% 
  summarise(n = n()) %>% 
  mutate(percent = n / sum(n) * 100)

PP.true <- PP.summary %>% 
  filter(ProperPause == TRUE) %>% 
  pull()

PP.false <- PP.summary %>% 
  filter(ProperPause == FALSE) %>% 
  pull()

PP.ratio <- round((PP.true / PP.false), 2)

```

Of all pauses that were started, `r round(PP.summary[which(PP.summary$ProperPause == FALSE),]$percent, 2)`%  were not actively terminated but lasted for their default maximum length of 3 minutes and therefore do not qualify as proper pauses according to Hüske-Kraus et al. (2018). The ICU’s proper pause to pause ratio is `r PP.ratio`:1.
